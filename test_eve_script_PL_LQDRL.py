...
def plot_uav_trajectory(env, uav_trajectory, layer, ep, t):
    ...
    plt.legend()
    plt.savefig(f'multi_layer_outputs/plots_multi_layer/randomtest9/{layer+1}_layers_uav_trajectory_{ep}_timestep_{t}.png')
    plt.close()
...
for m in range(m_layers):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            for m in range(len(secrecy_rates)):
                sr = secrecy_rates[m]
            ...
            if len(buffer) >= batch_size:
                ...
                (loss_val, td_errors), grads = jax.value_and_grad(critic_loss, has_aux=True)(critic.theta)
                critic_losses.append(loss_val)

                updates, critic_opt_state = critic_opt.update(grads, critic_opt_state)
                critic.theta = optax.apply_updates(critic.theta, updates)

                buffer.batch_update(b_idx, np.array(td_errors))

                actor_loss_val, actor_grads = jax.value_and_grad(actor_loss)(actor.theta)
                actor_losses.append(actor_loss_val)
                updates, actor_opt_state = actor_opt.update(actor_grads, actor_opt_state)
                actor.theta = optax.apply_updates(actor.theta, updates)

                #grads = jax.grad(lambda th: critic_loss(th)[0])(critic.theta)
                grads = jax.grad(loss_val)(critic.theta)
                updates, critic_opt_state = critic_opt.update(grads, critic_opt_state)
                critic.theta = optax.apply_update(critic.theta, updates)

                buffer.batch_update(b_idx, np.array(td_errors))

                actor_loss = (actor_loss(actor.theta))
                actor_losses.append(actor_loss)
                grads = jax.grad(actor_loss)(actor.theta)
                updates, actor_opt_state = actor_opt.update(grads, actor_opt_state)
                actor.theta = optax.apply_update(actor.theta, updates)
                '''
    ...
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_rewards_over_episodes.png")
    plt.close()
    ...
    plt.ylabel("Loss")
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_layers_losses.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_layers_sum_rates.png")
    plt.close()
    '''
    ...
    for i in range(5):
        for j in range(2):
            for gu_id in range(len(env.legit_users)):
                ...
                ax[i, j].plot(gu_rates, color=colour_codes[gu_id])
                ax[i, j].set_xlabel("Timesteps")
                ax[i, j].set_ylabel("Secrecy Rates")
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_layers_secrecy_rates.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_layers_distances_to_centroid.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_layers_episodewise_rewards.png")
    plt.close()
    '''
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_layers_reward_vs_distance.png")
    plt.close()
    '''
    ...
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_global_energy_eff.png")
    plt.close()
    ...
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_global_energy_cons.png")
    plt.close()
    ...
    plt.ylabel("Sum Rate")
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_global_sum_rates_perGU.png")
    plt.close()
    ...
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_timestep_energy_eff.png")
    plt.close()
    ...
    plt.xlabel("Timestep")
    plt.ylabel("Energy Consumption")
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_timestep_energy_cons.png")
    plt.close()
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax[i, j].set_xlabel("Timestep")
            ax[i, j].set_ylabel("Sum Rate")
            ax[i, j].legend()
            idx += 1
    fig.suptitle(f"Maximum, Minimum & Mean Sum Rates for All GUs")
    #fig.legend(gu_labels, loc='upper right', ncol=len(env.legit_users), fontsize=12)
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_timestep_sum_rate_all_GUs.png")
    plt.close()
    ...
    for gu in range(env.num_legit_users):
        ...
        plt.xlabel("Timestep")
        plt.ylabel("Sum Rate")
        plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_timestep_sum_rate_GU{gu}.png")
        plt.close()
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax[i, j].set_xlabel("Timestep")
            ax[i, j].set_ylabel("Secrecy Rate")
            ax[i, j].legend()
            idx += 1
    fig.suptitle(f"Maximum, Minimum & Mean Secrecy Rates for All GUs")
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_timestep_secrecy_rate_all_GUs.png")
    plt.close()
    ...
    for gu in range(env.num_legit_users):
        ...
        plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_timestep_secrecy_rate_GU{gu}.png")
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_episode_sum_rate_stats.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_episode_secrecy_rate_stats.png")
    plt.close()
    ...
    plt.xlabel("Episode")
    plt.ylabel("Energy Efficiency")
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_episode_energy_eff.png")
    plt.close()
    ...
    plt.xlabel("Episode")
    plt.ylabel("Energy Consumption")
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_episode_energy_cons.png")
    plt.close()
    ...
    plt.xlabel("Episode")
    plt.ylabel("Reward")
    plt.savefig(f"multi_layer_outputs/plots_multi_layer/randomtest9/{m+1}_episode_extrapolated_rewards.png")
    plt.close()
